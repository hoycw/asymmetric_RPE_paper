function BHV02d_RL_model_plot_grp_predictions(SBJ_id,proc_id,model_id,plt_id,save_fig,varargin)
...
%% Plot Predictors
fig_name = [SBJ_id '_' mdl.model_id '_predictions'];
...
if strcmp(mdl.model_lab,'RSVPE')
    model_str = 'Reward Valence vs. Value vs. Prediction Error';
elseif any(strcmp(mdl.model_lab,{'VML','SML','VSML'}))
    model_str = 'Outcome Features';
    plt.leg_loc = 'southwest';
elseif any(strcmp(mdl.model_lab,{'ERPEsL','RPEsL','ERPEs'}))
    model_str = 'Reward Prediction Error (RPE) Features';
else
    model_str = mdl.model_id;
end
title(model_str,'Interpreter','none');
if strcmp(mdl.model_id,'ERPEsL_pW25_DifFB'); plt.leg_loc = 'southwest'; end
