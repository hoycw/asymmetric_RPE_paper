function [data_all] = fn_read_neuralynx_interp(fname)
% Colin W. Hoy editted this function on May 21 2021 to add
%   checks for invalid samples based on repeated timestamps,
%   then remove them before interpolation
...
%   % replace repeat data with NaN
%   for bad_ix = 1:numel(repeat_timestamps)
%       time_ix = find(ts==repeat_timestamps(bad_ix));
%       for t_ix = 1:numel(time_ix)
%         data.trial{1}(time_ix(t_ix):time_ix(t_ix)+record_ts_gap) = nan;
%         ts(time_ix(t_ix):time_ix(t_ix)+record_ts_gap) = nan;
%       end
%   end
  ...
  % check for and remove repeats
  [~,idx2orig,idx2new] = unique(ts,'first'); % take index of first occurence
  if ~isempty(setdiff(idx2new,idx2orig))
      ts = ts(idx2orig);
      data.trial{1} = data.trial{1}(idx2orig);
      data.sampleinfo = [1 numel(data.trial{1})];
      fprintf(2,'\tWarning: tossing %d samples for repeating timestamps\n',numel(setdiff(idx2new,idx2orig)));
  end
