function SBJ08f_HFA_plot_grp_GLM_onsets_reg(SBJ_id,proc_id,an_id,model_id,stat_id,roi_id,...
                                            plt_id,save_fig,varargin)%gm_thresh,z_thresh,
error('idea is to flip this and show within INS does uRPE or sRPE come first, but not worth pre-exit talk');
...
%         '_GM' num2str(gm_thresh) '_z' num2str(z_thresh) '_normRTout'];
    ...
    for roi_ix = good_roi_ix{reg_ix}
        ...
        if strcmp(plt.violin_scat_colors,'SBJ')
    ...
    %% Save figure
    if save_fig
        fig_dir = [root_dir 'PRJ_Error/results/HFA/onsets_ROI/'...
