function SBJ08e_HFA_plot_grp_GLM_reg_venn_ROI(SBJ_id, proc_id, stat_regs, hemi, roi_id,...
                                       plot_out, plt_id, save_fig, varargin)
%% Plot venn diagrams of overlap in mass GLM effects per ROI
...
%% Prep report
% Create output dir
save_dir = [root_dir 'PRJ_Error/results/HFA/GRP/model_reg_venn/' stat_id_cat ...
...
venn_legend = cell(size(stat_regs));
...
for main_ix = 1:numel(stat_regs)
    ...
    venn_legend{grp_ix} = reg_labs{main_ix};
...
fig_name = [SBJ_id '_venn_' atlas_id '_' roi_id '_' stat_id_cat];
