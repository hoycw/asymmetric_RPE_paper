...
ylim_fudge = (max_beta-min_beta)*plt_vars.ylim_fudge;
...
for ch_ix = 1:numel(beta.label)
    fig_name = [SBJ '_' stat_id '_' beta.label{ch_ix}];
    ...
    for grp_ix = 1:numel(grp_lab)
        main_lines(grp_ix) = plot(beta.time,squeeze(beta.trial(grp_ix,ch_ix,:)),...
    ...
    for grp_ix = 1:numel(grp_lab)
        if any(squeeze(beta.qval(grp_ix,ch_ix,:))<=st.alpha)
            ...
            if strcmp(plt_vars.sig_type,'bold')
                sig_chunks = fn_find_chunks(squeeze(beta.qval(grp_ix,ch_ix,:))<=st.alpha);
                sig_chunks(squeeze(beta.qval(grp_ix,ch_ix,sig_chunks(:,1)))>st.alpha,:) = [];
                for sig_ix = 1:size(sig_chunks,1)
                    if diff(sig_chunks(sig_ix,:))==0
                        scatter(beta.time(sig_chunks(sig_ix,1)),squeeze(beta.trial(grp_ix,ch_ix,sig_chunks(sig_ix,1))),...
                        ...
                        line(beta.time(sig_chunks(sig_ix,1):sig_chunks(sig_ix,2)),...
                            squeeze(beta.trial(grp_ix,ch_ix,sig_chunks(sig_ix,1):sig_chunks(sig_ix,2))),...
            ...
            fprintf('%s %s -- NO SIGNIFICANT CLUSTERS FOUND...\n',...
                beta.label{ch_ix},grp_lab{grp_ix});
        end
    end
